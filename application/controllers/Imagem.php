    ...
    public function deletar($veiculo_id, $foto_id) {
        if (($veiculo_id > 0 && $foto_id > 0)) {
            ...
            if ($this->Imagem_Model->delete($foto_id) > 0) {
                ...
                $this->session->set_flashdata('retorno', '<div class="alert alert-danger"><i class="far fa-hand-paper"></i> Falha ao Deletar Imagem...</div>');
            }
        } else {
            $this->session->set_flashdata('retorno', '<div class="alert alert-danger">Veículo não encontrado</div>');
        }
